...
import LocketCommentMessage from "@/components/Inbox/LocketCommentMessage";
        ...
        <div className="w-full shadow flex flex-col justify-start items-start px-4 py-3 rounded-xl bg-white dark:bg-zinc-800 text-black dark:text-gray-300">
          ...
          <div className="w-full h-auto flex flex-col justify-start items-start px-2 pb-4">
            ...
            <div className="flex flex-col justify-start items-start w-full mt-4 gap-4">
              {inboxes.map((inbox) => {
                if (inbox.type === Inbox_Type.TYPE_LOCKET_COMMENT) {
                  return <LocketCommentMessage key={`${inbox.name}-${inbox.status}`} inbox={inbox} />;
