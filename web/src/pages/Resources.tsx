...
const Resources = () => {
  ...
  const groupedResources = groupResourcesByDate(filteredResources.filter((resource) => resource.locket));
  const unusedResources = filteredResources.filter((resource) => !resource.locket);

  useEffect(() => {
    resourceServiceClient.listResources({}).then(({ resources }) => {
      setResources(resources);
      loadingState.setFinish();
      Promise.all(resources.map((resource) => (resource.locket ? locketStore.getOrFetchLocketByName(resource.locket) : null)));
    });
  }, []);
  ...
  const handleDeleteUnusedResources = () => {
    showCommonDialog({
      ...
      onConfirm: async () => {
        ...
        setResources(resources.filter((resource) => resource.locket));
      },
    });
  };
                        ...
                        <div key={timestamp} className="w-full flex flex-row justify-start items-start">
                          ...
                          <div className="w-full max-w-[calc(100%-4rem)] sm:max-w-[calc(100%-6rem)] flex flex-row justify-start items-start gap-4 flex-wrap">
                            {resources.map((resource) => {
                              const relatedLocket = resource.locket ? locketStore.getLocketByName(resource.locket) : null;
                              return (
                                  ...
                                  <div className="w-full max-w-full flex flex-row justify-between items-center mt-1 px-1">
                                    ...
                                    {relatedLocket && (
                                      <Link
                                        ...
                                        to={`/m/${relatedLocket.uid}`}
                                      >
                                        #{extractLocketIdFromName(relatedLocket.name)}
                                      </Link>
                                    )}
                                  </div>
                                </div>
