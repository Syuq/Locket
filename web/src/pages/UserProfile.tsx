...
import { useLocketList, useLocketStore, useUserStore } from "@/store/v1";
        ...
        {!loadingState.isLoading &&
          (user ? (
            <>
              ...
              <LocketFilter className="px-2 pb-3" />
              {sortedLockets.map((locket) => (
                <LocketView key={`${locket.name}-${locket.displayTime}`} locket={locket} showVisibility showPinned />
              ))}
              {isRequesting ? (
