...
import LocketResource from "./LocketResource";
import showPreviewImageDialog from "./PreviewImageDialog";
import SquareDiv from "./kit/SquareDiv";

const LocketResourceListView = ({ resources = [] }: { resources: Resource[] }) => {
    ...
    return (
      <div className="w-full flex flex-row justify-start flex-wrap gap-2">
        {otherResources.map((resource) => (
          <LocketResource key={resource.name} resource={resource} />
        ))}
      </div>
    );
  };
...
export default memo(LocketResourceListView);
