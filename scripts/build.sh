#!/bin/bash
# This script builds lockets for all listed platforms.
...
# ./scripts/build.sh
#
# Output: ./build/lockets-<os>-<arch>[.exe]
...
for build in "${goBuilds[@]}"; do
    ...
    output="$repo_root/build/lockets-$os-$arch"
    if [ "$os" = "windows" ]; then
        output="$output.exe"
    fi

    CGO_ENABLED=0 GOOS=$os GOARCH=$arch go build -trimpath -ldflags="${ldFlags[*]}" -o "$output" ./bin/lockets/main.go

    echo -e "\033[34mBuilding $os/$arch to $output...\033[0m"
    GOOS=$os GOARCH=$arch go build -ldflags="${ldFlags[*]}" -o "./build/lockets-$os-$arch" ./bin/lockets/main.go
    if [ $? -ne 0 ]; then
...
echo -e "\n\033[32mYou can test the build with \033[37m./build/lockets-<os>-<arch>\033[0m\033[90m.exe\033[0m \033[37m--mode demo\033[0m"
