...
func (d *DB) UpsertUserSetting(ctx context.Context, upsert *storepb.UserSetting) (*storepb.UserSetting, error) {
	...
	} else if upsert.Key == storepb.UserSettingKey_USER_SETTING_LOCKET_VISIBILITY {
		valueString = upsert.GetLocketVisibility()
...
func (d *DB) ListUserSettings(ctx context.Context, find *store.FindUserSetting) ([]*storepb.UserSetting, error) {
	...
	for rows.Next() {
		...
		} else if userSetting.Key == storepb.UserSettingKey_USER_SETTING_LOCKET_VISIBILITY {
			userSetting.Value = &storepb.UserSetting_LocketVisibility{
				LocketVisibility: valueString,
			}
