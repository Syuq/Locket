...
func (d *DB) ListLocketRelations(ctx context.Context, find *store.FindLocketRelation) ([]*store.LocketRelation, error) {
	...
	if find.RelatedLocketID != nil {
		where, args = append(where, "related_locket_id = "+placeholder(len(args)+1)), append(args, find.RelatedLocketID)
	}
	if find.Type != nil {
...
func (d *DB) DeleteLocketRelation(ctx context.Context, delete *store.DeleteLocketRelation) error {
	...
	if delete.LocketID != nil {
		where, args = append(where, "locket_id = "+placeholder(len(args)+1)), append(args, delete.LocketID)
	}
	if delete.RelatedLocketID != nil {
		where, args = append(where, "related_locket_id = "+placeholder(len(args)+1)), append(args, delete.RelatedLocketID)
	}
	if delete.Type != nil {
