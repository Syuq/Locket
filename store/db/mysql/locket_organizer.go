...
func (d *DB) UpsertLocketOrganizer(ctx context.Context, upsert *store.LocketOrganizer) (*store.LocketOrganizer, error) {
	stmt := "INSERT INTO `locket_organizer` (`locket_id`, `user_id`, `pinned`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `pinned` = ?"
	if _, err := d.db.ExecContext(ctx, stmt, upsert.LocketID, upsert.UserID, upsert.Pinned, upsert.Pinned); err != nil {
		return nil, err
	}
	return upsert, nil
}
...
func (d *DB) ListLocketOrganizer(ctx context.Context, find *store.FindLocketOrganizer) ([]*store.LocketOrganizer, error) {
	...
	query := "SELECT `locket_id`, `user_id`, `pinned` FROM `locket_organizer` WHERE " + strings.Join(where, " AND ")
...
func vacuumLocketOrganizer(ctx context.Context, tx *sql.Tx) error {
	stmt := "DELETE FROM `locket_organizer` WHERE `locket_id` NOT IN (SELECT `id` FROM `locket`) OR `user_id` NOT IN (SELECT `id` FROM `user`)"
