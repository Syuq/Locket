...
func (d *DB) UpsertLocketRelation(ctx context.Context, create *store.LocketRelation) (*store.LocketRelation, error) {
	stmt := "INSERT INTO `locket_relation` (`locket_id`, `related_locket_id`, `type`) VALUES (?, ?, ?)"
	...
	locketRelation := store.LocketRelation{
		LocketID:        create.LocketID,
		RelatedLocketID: create.RelatedLocketID,
		Type:          create.Type,
	}
...
func (d *DB) ListLocketRelations(ctx context.Context, find *store.FindLocketRelation) ([]*store.LocketRelation, error) {
	...
	rows, err := d.db.QueryContext(ctx, "SELECT `locket_id`, `related_locket_id`, `type` FROM `locket_relation` WHERE "+strings.Join(where, " AND "), args...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (d *DB) DeleteLocketRelation(ctx context.Context, delete *store.DeleteLocketRelation) error {
	...
	stmt := "DELETE FROM `locket_relation` WHERE " + strings.Join(where, " AND ")
...
func vacuumLocketRelations(ctx context.Context, tx *sql.Tx) error {
	if _, err := tx.ExecContext(ctx, "DELETE FROM `locket_relation` WHERE `locket_id` NOT IN (SELECT `id` FROM `locket`) OR `related_locket_id` NOT IN (SELECT `id` FROM `locket`)"); err != nil {
		return err
	}
	return nil
}
